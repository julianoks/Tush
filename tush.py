...
class Tush(object):
    ...
    def execute_step(self, stacks):
        ...
        try:
            if(Instructions[instr]['out_type']=='chk_stack'):
                out = Instructions[instr]['fn'](*[i for i in inputs])
                inp_dep = any([i['input_dep'] for i in inputs]) or instr_inp_dep
                var_dep = any([i['variable_dep'] for i in inputs]) or instr_var_dep
                out_stack = out['stack']
                
                stacks[out_stack].append({'val': out['val'], 'stack': out_stack, 'input_dep': inp_dep, 'variable_dep': var_dep})
...
#             logging.debug(shape)
            ...
            if(len(list(out.shape))>len(shape) and list(out.shape)[0]==shape[0]):
                pass
